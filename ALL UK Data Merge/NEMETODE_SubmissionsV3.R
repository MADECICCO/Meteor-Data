NEMETODE_Submissions <<- function() {
        ##
        ## This function reads in a consolidated .csv file of NEMETode data and creates a table of the number
        ...
        ALL_Data <<- read.csv(paste("NEMETODE Consolidated Data_",Curr_Year,Curr_Month,Curr_Day, ".csv",sep=""))
        #ALL_Data <<- read.csv(paste("NEMETODE Consolidated Data_201659.csv",sep=""))
        ...
        write.csv(Cameras,file = paste("NEMETODE Events Submitted per Camera per Month.csv",sep=""), quote=FALSE,row.names=FALSE)
